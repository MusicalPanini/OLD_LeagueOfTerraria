...
namespace TerraLeague.Items.SummonerSpells
{
    public class GarrisonRune : SummonerSpell
    {
        ...
        static public void Efx(Player player)
        {
            ...
            for (int j = 0; j < 18; j++)
            {
                int num2 = Dust.NewDust(new Vector2(Main.rand.Next((int)player.position.X - 8, (int)player.position.X + 8), player.position.Y + 16), player.width, player.height, 261, 0, -Main.rand.Next(6, 18), 0, new Color(131, 234, 46, 0), Main.rand.Next(2, 3));
