...
namespace TerraLeague.Projectiles
{
    public class MagicArrowProj : ModProjectile
    {
        ...
        {
            int dustIndex = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 113, 0f, 0f, 200, default(Color), 0.5f);
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (!target.immortal)
            {
                Projectile proj = Projectile.NewProjectileDirect(target.Center, new Vector2(0, 6).RotatedByRandom(MathHelper.TwoPi), ProjectileType<EchoShot>(), projectile.damage/2, 0, projectile.owner, target.whoAmI);
            }
