...
{
    public class HextechChloroShot : ModProjectile
    {
        bool split = false;
            ...
            if (!split)
            {
                if (num == -1)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            Projectile.NewProjectileDirect(projectile.position, projectile.velocity.RotatedByRandom(MathHelper.TwoPi) / 12, ProjectileType<HextechChloroShotSplit>(), projectile.damage / 2, 0, projectile.owner, num == -1 ? 255 : num);
                        }
                        else
                        {
                ...
                else
                {
                    for (int i = 0; i < 3; i++)
                    {
                        if (Main.rand.Next(3) == 0)
                        {
                            Projectile.NewProjectileDirect(projectile.position, projectile.velocity.RotatedBy(MathHelper.ToRadians(-15 + (15 * i))) / 12, ProjectileType<HextechChloroShotSplit>(), projectile.damage / 2, 0, projectile.owner, num == -1 ? 255 : num);
