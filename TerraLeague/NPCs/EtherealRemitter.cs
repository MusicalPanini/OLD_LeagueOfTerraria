    ...
    public class EtherealRemitter : ModNPC
    {
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            else
            {
                for (int i = 0; i < 20; i++)
                {
                    int num620 = Dust.NewDust(npc.position, npc.width, npc.height, 54, 0f, 0f, 50, default(Color), 1.5f);
                ...
                gore.velocity *= 0.3f;
                num621 = Gore.NewGore(new Vector2(npc.position.X, npc.position.Y + (float)(npc.height / 2) - 15f), new Vector2((float)hitDirection, 0f), 99, npc.scale);
